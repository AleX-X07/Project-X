...
AiMobSpawner_Desert::AiMobSpawner_Desert(Object* _owner, std::vector<Object*>& _sceneObjects) : Component(_owner), sceneObjects(_sceneObjects) {
...
AiMobSpawner_Desert::~AiMobSpawner_Desert()
{
    for (auto c : liste)
    {
        delete c;
...
void AiMobSpawner_Desert::update(float dt)
{
    actualtime += dt;
...
void AiMobSpawner_Desert::render()
{
    for (auto c : liste)
        c->render();
    for (auto b : ExpList)
...
void AiMobSpawner_Desert::addMob()
{
    ...
    else if (r <= 50)
    {
        ...
        Mob->addComponent(new GoldContainer(Mob, 15, 100));
        Mob->addComponent(new ChemicalManager(Mob));
        Mob->addComponent(new ChemicalRenderer(Mob));
...
void AiMobSpawner_Desert::giveExp() {
